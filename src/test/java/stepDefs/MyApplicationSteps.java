...
public class MyApplicationSteps {
    ...
    public void user_gets_employee_info_for_employee_id_from_UI(String employee_id) throws Throwable {
            int indexOfRow = 0;
            for (int i = 0; i < maPage.empoyee_ids.size(); i++) {
                if (maPage.empoyee_ids.get(i).getText().equalsIgnoreCase(employee_id)) {
                    indexOfRow=i;
                }
            }
            //table//tr[]//td
        ...
        @When("^User gets data fro employee_id \"([^\"]*)\" from database$")
        public void user_gets_data_fro_employee_id_from_database (String employee_id) throws Throwable {
